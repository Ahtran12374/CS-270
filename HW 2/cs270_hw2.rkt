...
(define (all_q L)
  (if (null? L)
      #t
      (andi ((lambda (x) (equal? x 'q)) (first L)) (all_q(rest L)))
      )
)
...
(define (all_q_v2 L)
  (andlist (map (lambda (L) (equal? L 'q)) L))
)
...
(define (at_least_one_q L)
  (if (null? L)
      #f
      (ori (equal? (first L) 'q) (at_least_one_q(rest L)))
      )
)
...
(define (at_least_one_q_v2 L)
  (orlist (map (lambda (L) (equal? L 'q)) L))
)
...
(define (exactly_one_q L)
  (if (null? L)
      ...
      (if (equal? 'q (first L))
          (if (null? (rest L))
              #t
              (if (ori (equal? (first (rest L)) 'q) (at_least_one_q (rest(rest L))))
                  #f
                  #t)
              )
...
(define (odd_amt_q L)
  (if (null? L)
      #f
      (xori (equal? (first L) 'q) (odd_amt_q(rest L)))
      )
)
...
(define (odd_amt_q_v2 L)
  (foldr xori #f (map (lambda (L) (equal? L 'q)) L))
  )
